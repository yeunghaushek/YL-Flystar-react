...
export default function Astrolabe() {
    ...
    const routeFlows = useMemo(() => {
        ...
        comps.forEach((comp) => {
            ...
            // Choose primary by longest length (stable and predictable)
            items.sort((a, b) => b.seq.length - a.seq.length);
            ...
            const internalSuffixOverlap = (A, B) => {
                ...
                return internalSuffixOverlapRaw(A, B);
            };
            const internalPrefixOverlap = (A, B) => {
                ...
                // fallback to raw when no context-key match
                return internalPrefixOverlapRaw(A, B);
            };
            ...
            while (mergedSomethingOnce && remaining.length > 0) {
                ...
                for (let r = 0; r < remaining.length; r++) {
                    ...
                    const L1 = longestOverlap(curPrimarySeq, seq); // primary suffix vs seq prefix
                    if (L1 >= 1 && seq.length > L1) {
                        const tail = seq.slice(L1);
                        appendToMain(tail);
                    ...
                    const L2 = longestOverlap(seq, curPrimarySeq); // seq suffix vs primary prefix
                    if (L2 >= 1 && curPrimarySeq.length > L2) {
                        const head = seq.slice(0, seq.length - L2);
